...
DROP TABLE IF EXISTS {{.output_schema}}.sessions_tmp;
CREATE TABLE {{.output_schema}}.sessions_tmp
...
AS (
  ...
  WITH prep AS (
    ...
    FROM {{.output_schema}}.page_views_tmp
  ...
  FROM {{.output_schema}}.page_views_tmp AS a
